...
from ._client import Fault, dumps, loads, gzip_encode, gzip_decode
    ...
    def handle_request(self, environ, start_response, request_text=None):
        ...
        if request_text is None and environ.get('REQUEST_METHOD', None) == 'GET':
            ...
            if pq:
                ...
                pq = [environ.get('PATH_INFO', ''), environ.get('QUERY_STRING', '')]
            uri = pq[0]
            #pprint(pq)
            #if pq[0] and pq[0][-1] == '/':
            #    pq[0] = pq[0][:-1]
...
def test_wsgi():
    ...
    handler.register_function(pow)
    handler.register_function(lambda x,y: float(x)+float(y), 'add')
    handler.register_instance(MyFuncs())
